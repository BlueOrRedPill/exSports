...
data class ResBcTokensOwned(
    @SerializedName("data")
    override val result: BaseData,
    override val warnings: List<String>,
    override val status: Integer,
    override val provider: String ) : NewBaseResponse<List<CardModel>> {
    override val data: List<CardModel>
        get() = CardModel.fromJsonList(result.data.getAsJsonArray("cardList").toString())
